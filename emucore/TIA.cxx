...
//  myCurrentPFMask = ourPlayfieldTable[0];
  ...
  {
    std::cerr << "Unknown error in save state for " << device << std::endl;
    return false;
  }
...
//  myCurrentPFMask = ourPlayfieldTable[0];
  ...
  {
    std::cerr << "Unknown error in load state for " << device << std::endl;
    return false;
  }
  ...
  do
  {
    ...
    if(myClocksToEndOfScanLine == 228)
    {
      ...
      if(myM0CosmicArkMotionEnabled)
      {
        ...
        static const uInt32 m[4] = {18, 33, 0, 17};
...
uInt8 TIA::INPT0_3(const uInt8 &noise, const Int32 &r)
{
...
uInt8 TIA::peek(uInt16 addr)
{
  ...
  const uInt8 noise = mySystem->getDataBusState() & 0x3F;
...
void TIA::poke(uInt16 addr, uInt8 value)
{
  ...
  switch(addr)
  {
    ...
    {
      const Int32 hpos = (clock - myClockWhenFrameStarted) % 228;
      const Int32 newx = hpos < HBLANK ? 3 : (((hpos - HBLANK) + 5) % 160);

      // Find out under what condition the player is being reset
      const Int8 when = ourPlayerPositionResetWhenTable[myNUSIZ0 & 7][myPOSP0][newx];
    ...
    {
      const int hpos = (clock - myClockWhenFrameStarted) % 228;
    ...
    case 0x14:    // Reset Ball
    {
      ...
      // occurs 18, 3, 7 and 6 cycles after an HMOVE (01/09/99) (04/13/02).

      const Int32 clockHMOVE = clock - myLastHMOVEClock;
    ...
    case 0x1C:    // Graphics Player 1
    {
      ...
      const uInt8 grp0 = myVDELP0 ? myDGRP0 : myGRP0;
      myCurrentGRP0 = myREFP0 ? ourPlayerReflectTable[grp0] : grp0;

      // Get the "current" data for GRP1 base on delay register
      const uInt8 grp1 = myVDELP1 ? myDGRP1 : myGRP1;
      myCurrentGRP1 = myREFP1 ? ourPlayerReflectTable[grp1] : grp1;
    ...
    case 0x2A:    // Apply horizontal motion
    {
      ...
      const Int32 x = ((clock - myClockWhenFrameStarted) % 228) / 3;
    ...
    {
      memset((void *)&myHMP0, 0, sizeof(uInt8) * 5);
      break;
    }
    ...
    default:
    {
      ...
      std::cerr << "BAD TIA Poke: " << hex << addr << std::endl;
#endif
      break;
    }
  }
